 ...
 * can go through without any change to the original value. Also, converters are chained, so that the result of one is
 * passed to the next and so one, whereas with adapters, you have a single adapter suitable for the current situation
 * that operates on the result.</p>
 *
