...
public interface DataStore<K, E> {
     ...
     * data store in some sort of mapping does not matter. The only expectation is that after this call, should the save
     * be successful a call to {@link #hasKey(Object)} should yield {@literal true} and the very same entity (or an
     * equal value entity) can be retrieved by calling {@link #retrieve(Object)}.
     *
     * @param key    the key
     ...
     * that as a side effect, once this method returns successfully, {@link #hasKey(Object)} should return {@literal
     * false} and {@link #retrieve(Object)} should return {@literal null} for the same key.
     *
     * @param key the key for which the removal should happen
     * @return {@literal true} to indicate the entity was located and removed and {@literal false} to indicate that no
     * such entity existed in the first place.
     */
