 ...
 * @since 1.0 (10/8/15)
 */
public class RepositoryFactoryConfigurationBuilder implements QueryDescriptionExtractorConfigurer, OperatorContextConfigurer {
    ...
    private RepositoryFactoryConfigurationBuilder() {
        this(new DefaultDataFunctionRegistry(), new DefaultDataStoreRegistry(), new DefaultResultAdapterContext(), new DefaultTypeMappingContext(), new QueryDescriptionExtractor(new DefaultOperatorContext()), new DefaultRepositoryMetadataResolver());
    }

    private RepositoryFactoryConfigurationBuilder(DataFunctionRegistry functionRegistry, DataStoreRegistry dataStoreRegistry, ResultAdapterContext adapterContext, TypeMappingContext typeMappingContext, QueryDescriptionExtractor descriptionExtractor, RepositoryMetadataResolver repositoryMetadataResolver) {
    ...
    public DataStoreConfigurer withFunctions(DataFunctionRegistry functionRegistry) {
        return new RepositoryFactoryConfigurationBuilder(functionRegistry, dataStoreRegistry, adapterContext, typeMappingContext, descriptionExtractor, repositoryMetadataResolver);
    }
    ...
    public MetadataResolverConfigurer withOperators(OperatorContext operatorContext) {
        return new RepositoryFactoryConfigurationBuilder(functionRegistry, dataStoreRegistry, adapterContext, typeMappingContext, new QueryDescriptionExtractor(operatorContext), repositoryMetadataResolver);
    }

    @Override
    public RepositoryFactoryConfiguration configure() {
        return new ImmutableRepositoryFactoryConfiguration(functionRegistry, dataStoreRegistry, adapterContext, typeMappingContext, descriptionExtractor, repositoryMetadataResolver);
    }
